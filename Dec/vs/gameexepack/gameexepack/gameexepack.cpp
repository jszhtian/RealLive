// gameexepack.cpp : 定义控制台应用程序的入口点。
//

#include "stdafx.h"

typedef unsigned char byte;
typedef unsigned short word;
unsigned char* buf;
unsigned char* data;
typedef unsigned int ulong;


void decrypt_1(byte* debuf, size_t desize)
{
	size_t key_idx = 0;
	size_t xor_idx = 0;
	static byte key[] =
	{
		0xB9, 0x1D ,0x47 ,0xE6 ,0xBD ,0x0D ,0x09 ,0xBF ,0xE7 ,0xE8 ,0xFD ,0x7E ,0xAC ,0x17 ,0xFD ,0x3A
	};

	for (xor_idx = 0; xor_idx < desize; xor_idx++, key_idx++, key_idx = key_idx & 0x8000000F)
	{
		debuf[xor_idx] ^= key[key_idx];
	}
}

void decrypt_2(byte* debuf, size_t desize)
{
	size_t key_idx = 0;
	size_t xor_idx = 0;

	static byte key[] =
	{
		
	};
	for (xor_idx = 0; xor_idx < desize; xor_idx++, key_idx++, key_idx = key_idx & 0x800000FF)
	{
		debuf[xor_idx] ^= key[key_idx];
	}
}

int main(int argc, char** args)
{
	char* file_name;
	if (argc > 1)
	{
		file_name = args[1];
	}
	else
		return 0;
	FILE* f = fopen(file_name, "rb");
	fseek(f, 0, SEEK_END);
	unsigned int size = ftell(f);
	fseek(f, 0, SEEK_SET);
	//buf = (unsigned char*)malloc(size);
	//fread(buf, size, 1, f);
	fseek(f, 0, 0);
	data = (unsigned char*)malloc(size);
	fread(data, size, 1, f);
	int size1 = size;
	int size2 = size;
	buf = (unsigned char*)malloc(size + 8);
	memcpy(buf, &size1, sizeof(size1));
	memcpy(buf+sizeof(size1), &size2, sizeof(size2));
	memcpy(buf + sizeof(size1) + sizeof(size2), data, size);



	decrypt_1(buf, size);
	decrypt_2(buf, size);


	FILE* f2 = _wfopen(L"outgameexe.dat", L"ab+");
	int head = 0x0;
	int flag = 0x1;

	fwrite(&head, 0x04, 1, f2);
	fwrite(&flag, 0x04, 1, f2);

	fwrite(buf, size, 1, f2);
	fclose(f2);





	return 0;
}
